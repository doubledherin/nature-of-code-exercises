...
function setup() {
  const p = createP(
    "Exercise 0.8: In graphing 2D noise, play with color, noiseDetail(), and the rate at which xoff and yoff are incremented to achieve different visual effects."
  )
  p.position(10, 0)
...
function showCode() {
  window.open(
    "https://github.com/doubledherin/nature-of-code-exercises/blob/main/exercises/0-Randomness/0.8/script.js",
    "_blank"
  )
}
...
function draw() {
  ...
  for (let x = 0; x < width; x++) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      const red = map(noise(xoff, yoff), 0, 1, 0, redSlider.value())
      const green = map(noise(xoff, yoff), 0, 1, 0, greenSlider.value())
      const blue = map(noise(xoff, yoff), 0, 1, 0, blueSlider.value())
      pixels[index] = red
